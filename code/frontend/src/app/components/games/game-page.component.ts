...
export class GamePageComponent implements OnInit {
  ...
  gameplayImages() {
    ...
    this.gameInfo.game.gameplayImages.forEach((_) => {
      images.push(
        this.gameService.getGameplayImage(this.gameInfo.game, index + 1)
      );
      index++;
    });
    return images;
  ...
  getMoreReviews() {
    ...
    this.reviewService
      .getMoreReviews(this.gameInfo.game.id, this.indexReviews)
      .subscribe((response) => {
  ...
  gameInCart() {
    return this.userProfile.user.cart.some(
      (game) => game.id === this.gameInfo.game.id
    );
  }

  gameBought() {
    return this.userProfile.games.some(
      (game) => game.id === this.gameInfo.game.id
    );
  }
  ...
  reviewGame() {
    ...
    this.reviewService
      .reviewGame(this.gameInfo.game.id, this.userProfile.user.id, formData)
      .subscribe(
        (response) => {
          this.loadMainGame();
        },
  ...
  checkIfDelete(review: Review) {
    ...
    if (
      ...
      this.userProfile.user.id === review.user.id
    ) {
      return true;
    }
    return false;
  ...
  addToCart(event: any) {
    ...
    this.userService
      .addToCart(this.gameInfo.game.id, this.userProfile.user.id)
      .subscribe(
        (response) => {
          this.userProfile.user.cart = this.userProfile.user.cart.concat(
            response.content
          );
          this.inCart = true;
  ...
  loadGame(id: number) {
    this.gameService.getGameById(id).subscribe(
      (response) => {
        ...
        this.reviewService
          .isReviewed(this.gameInfo.game.id, this.userProfile.user.id)
          .subscribe((response) => {
